...
ML_MODEL_PKL_PATH = os.environ.get('ML_MODEL_PKL_PATH', 'trained_option_model.rules.pkl')
...
def init_ml_db():
    ...
    #     )
    # ''') # Model state (self.rules) will now be stored in a .pkl file
    conn.commit()
    conn.close()
...
class OptionChainMLModel:
    def __init__(self):
        self.rules = {}  # Initialize with a default empty ruleset
        self._load_state()

    def _load_state(self):
        """Loads the model's rules from a .pkl file."""
        if os.path.exists(ML_MODEL_PKL_PATH):
            try:
                with open(ML_MODEL_PKL_PATH, 'rb') as f:
                    loaded_rules = pickle.load(f)
                    if isinstance(loaded_rules, dict):
                        self.rules = loaded_rules
                        print(f"ML model rules loaded from {ML_MODEL_PKL_PATH}.")
                    else:
                        print(f"Warning: Data in {ML_MODEL_PKL_PATH} is not a dictionary. Using default rules.")
                        self.rules = {} # Fallback to default
            except Exception as e:
                print(f"Error loading ML model rules from {ML_MODEL_PKL_PATH}: {e}. Using default rules.")
                self.rules = {} # Fallback to default
        else:
            print(f"{ML_MODEL_PKL_PATH} not found. Starting with default rules.")
            self.rules = {} # Ensure it's initialized if file doesn't exist

    def _save_state(self):
        """Saves the model's rules to a .pkl file."""
        try:
            with open(ML_MODEL_PKL_PATH, 'wb') as f:
                pickle.dump(self.rules, f)
            print(f"ML model rules saved to {ML_MODEL_PKL_PATH}.")
        except Exception as e:
            print(f"Error saving ML model rules to {ML_MODEL_PKL_PATH}: {e}")

        # The following code for saving to DB is now replaced by .pkl saving for self.rules
        # If other parts of the model state needed DB persistence, that would remain.
